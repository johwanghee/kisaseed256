package com.github.johwanghee.encryption.kisaseed256;
...
public class KisaSeed256 {
    ...
    public static byte[] SeedEncrypt(byte[] password, String key) throws InvalidKeyException {
        if (key.getBytes().length != 32) {
            throw new InvalidKeyException("Illegal key size 32");
        }
    ...
    public static byte[] SeedEncrypt(String password, String key) throws InvalidKeyException {
        return SeedEncrypt(password.getBytes(), key);
    }

    public static String Decrypt(byte[] hash, String key) {
        ...
        if (hash.length <= TEXT_SIZE) {
            ...
            for (int i = 0; i < loop; i++) {
                if (TEXT_SIZE >= 0) System.arraycopy(hash, (i * TEXT_SIZE), temp2, 0, TEXT_SIZE);
                SeedDecrypt(temp2, pdwRoundKey, temp);
                System.arraycopy(temp, 0, result, (i * TEXT_SIZE), temp.length);
            }
     ...
     * @return
     */
    public static String Encrypt(String password, String key) throws InvalidKeyException {
        if (isEmpty(password)) {
            return null;
        }
        if (password.length() == 1) {
            ...
            if (pwdByte.length <= TEXT_SIZE) {
                temp = SeedEncrypt(AnsiX923Padding.addPadding(password.getBytes(), TEXT_SIZE), key);
                ...
                for (int i = 0; i < loop; i++) {
                    if ((pwdByte.length - end) <= TEXT_SIZE) {
                        ...
                        temp = SeedEncrypt(AnsiX923Padding.addPadding(new String(pwdByte, i * TEXT_SIZE, term).getBytes(), TEXT_SIZE), key);
                        System.arraycopy(temp, 0, result, (i * TEXT_SIZE), temp.length);
                    } else {
                        temp = SeedEncrypt(new String(pwdByte, i * TEXT_SIZE, term), key);
                        System.arraycopy(temp, 0, result, (i * TEXT_SIZE), temp.length);
                    }
                    end += TEXT_SIZE;
                }
            ...
            for (byte hash : result) {
                resultHash.append(addZero(Integer.toHexString(0xff & hash)));
            }
